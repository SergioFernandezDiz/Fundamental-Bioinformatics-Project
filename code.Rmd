...
```{r}
# 1. Calcoli principali (da eseguire una sola volta)
logcounts_all <- cpm(dgeObj, log = TRUE)
pca_res <- prcomp(t(logcounts_all), scale. = TRUE)
pca_df <- as.data.frame(pca_res$x)

# 2. Aggiunta metadati (collegando i campioni filtrati ai metadati md_gtex/md_tcga)
...
```{r}
# Grafico statico per il report (Dataset Distinction)
...
```{r}
# Grafico interattivo per l'esplorazione delle variabili
...
# Retrieve the conditions in our dataset (GTEx vs TCGA)
group <- factor(pca_df$Group, levels = c("GTEx", "TCGA"))

# Specify a design matrix with an intercept term (GTEx is the reference/baseline)
...
```{r}
# Three dispersion estimation steps to account for overdispersion in RNA-Seq data
dgeObj <- estimateCommonDisp(dgeObj)
dgeObj <- estimateGLMTrendedDisp(dgeObj)
dgeObj <- estimateTagwiseDisp(dgeObj)

# Visualize the Biological Coefficient of Variation (BCV)
plotBCV(dgeObj)
```
...
# Extract the most significantly differentially expressed genes
