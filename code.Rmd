...
print(brain_datasets[, c("name", "title", "version")])
```
...
```{r}
# Fetch the dataset
# Note: path should be one of the available paths from listPaths()
# If there's no path (NULL), don't specify it
cat("Downloading cortex single-cell dataset...\n")

# Try with path first
if(length(available_paths) > 0) {
  path <- available_paths[1]  # Use first available path
  cortex_sc <- fetchDataset(name, version, path)
} else {
  # If no paths, try without specifying path
  cortex_sc <- fetchDataset(name, version)
}
...
### 6.5.1. Extract Count Matrix
```{r}
# Get the count matrix
cat("Extracting count matrix...\n")
counts_matrix <- assay(cortex_sc, "counts")

cat("Count matrix dimensions:\n")
cat("Genes:", nrow(counts_matrix), "\n")
cat("Cells:", ncol(counts_matrix), "\n")

# Check for data quality issues
cat("\nChecking for duplicates and NA values...\n")
cat("Duplicate rownames (genes):", anyDuplicated(rownames(counts_matrix)), "\n")
cat("Duplicate colnames (cells):", anyDuplicated(colnames(counts_matrix)), "\n")
cat("NA in rownames:", sum(is.na(rownames(counts_matrix))), "\n")
cat("Empty rownames:", sum(rownames(counts_matrix) == ""), "\n")
cat("NA in colnames:", sum(is.na(colnames(counts_matrix))), "\n")
cat("Empty colnames:", sum(colnames(counts_matrix) == ""), "\n")

```

### 6.5.2. Handle Duplicate Gene Names
```{r}
# Make rownames (gene names) unique if needed
if(anyDuplicated(rownames(counts_matrix)) > 0) {
  cat("Making gene names unique...\n")
  rownames(counts_matrix) <- make.unique(rownames(counts_matrix))
  cat("Gene names are now unique.\n")
} else {
  cat("No duplicate gene names found.\n")
}

# Verify no duplicates remain
cat("Final check - Duplicate genes:", anyDuplicated(rownames(counts_matrix)), "\n")

```
## 6.6. Create Seurat Object
```{r}
# Create Seurat object
cat("Creating Seurat object...\n")
brain.seurat.obj <- CreateSeuratObject(
  counts = counts_matrix,
  project = "Brain_Cortex_scRNA",
